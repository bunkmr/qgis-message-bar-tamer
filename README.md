...
- **C++ 内核横幅全覆盖**：除了 monkey-patch Python 侧推送，还用 `QgsMessageBar.widgetAdded` 信号 **+ 事件过滤器** 双保险拦截 C++ 内核直接弹出的横幅（如**瓦片网络超时**、部分 CRS 提示）——这类走 C++ 层、绕过了 Python 补丁，v0.3 起已被覆盖，v0.4 起再加事件过滤器兜底，更稳。
...
### 方式二：从发布包安装
下载 `MessageBarTamer.0.4.zip`，解压到上述插件目录即可。
...
- **默认模式为「按级别过滤 + 保留 Critical」**：黄色 Info/Warning 横幅（含瓦片网络超时）到达即被移除；红色 Critical 报错照常保留。若想连红色也去掉，请在设置里切到「完全隐藏」或提高保留级别（自负后果）。已有安装会在升级到 v0.4 时自动从旧的「自动收起」默认迁移过来。
